...
from tools.spine_route_shape_parity import build_report, main, write_report
...
def test_route_envelopes_have_expected_top_level_keys(tmp_path):
    source = tmp_path / "source.db"
    _create_sample_db(source)
    report = build_report(source)
    validations = report["route_validations"]
    assert validations["open_queue"]["valid"] is True
    assert validations["blocked_queue"]["valid"] is True
    assert validations["approval_waiting_queue"]["valid"] is True
    assert validations["recent_events"]["valid"] is True
    assert validations["room_overview"]["valid"] is True
    assert validations["project_workload"]["valid"] is True
    assert validations["task_detail"]["valid"] is True


def test_secret_like_fields_are_redacted(tmp_path):
    source = tmp_path / "source.db"
    output = tmp_path / "report.json"
    _create_sample_db(source)
    ...
    approval = next(event for event in data["route_envelopes"]["recent_events"]["events"] if event["event_id"] == "evt-1")
...
def test_json_report_generated(tmp_path):
    ...
    assert data["route_envelopes"]["open_queue"]["count"] == 1
    assert data["route_envelopes"]["blocked_queue"]["count"] == 1


def test_no_disallowed_imports():
    mod = importlib.import_module("tools.spine_route_shape_parity")
